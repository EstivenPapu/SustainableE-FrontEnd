<template>
    <div class="search-container">
      ...
      <!-- Campo de búsqueda -->
      <input v-model="searchQuery" @keyup.enter="handleSearch" type="text" placeholder="Ingresa el nombre del país" class="search-input"/>
      ...
      <div v-if="countryData.length > 0" class="result">
        <div v-for="(data, index) in countryData" :key="index" class="country-card">
          ...
          <p><strong>Fuente de Energía:</strong> {{ data.fuenteDeEnergia.nombre }} ({{ data.fuenteDeEnergia.descripcion }})</p>
        </div>
      </div>
    </div>
  </template>
  ...
  const handleSearch = async () => {
    if (searchQuery.value.trim()) {
      ...
      error.value = 'Por favor ingresa un nombre de país.';
      countryData.value = [];
    }
  };
  </script>
